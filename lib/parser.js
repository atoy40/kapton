export default function parser(document) {
  ...
  if (queries.length && mutations.length && mutations.length) {
    if ((queries.length + mutations.length + mutations.length) > 1) {
      throw new Error(`Kapton supports a query, subscription, or a mutation per Behavior. ${document} had ${queries.length} queries, ${subscriptions.length} subscriptions and ${mutations.length} muations. You can use 'compose' to join multiple operation types to a component`);
    }
  }
  ...
  if (definitions.length !== 1) {
    throw new Error(`Kapton only supports one defintion per mixin. ${document} had ${definitions.length} definitions. You can use 'compose' to join multiple operation types to a component`);
  }
